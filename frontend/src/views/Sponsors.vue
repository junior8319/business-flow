  ...
  <main class="view-container">
    ...
    <section class="page-body">
      ...
      <article class="content-table">
        <ContentHead :table-head-labels="this.sponsorsLabels"/>

        <div
          v-for="sponsor in listOfSponsors"
          :key="sponsor.id"
          class="content-body"
        >
          <ContentBodyItem :value="sponsor.id" id="sponsor-id" />
          <ContentBodyItem v-if="sponsor.cnpj" :value="sponsor.cnpj.cnpj" />
          <ContentBodyItem :value="sponsor.tradingName" />
          <ContentBodyItem :value="sponsor.responsibleName" />
          <ContentBodyItem :value="sponsor.createdAt" />
          <ContentBodyItem :value="sponsor.updatedAt" />

          <FormUpdate
            v-if="sponsor.id === idOnFocus"
          ...
          >
            ...
            >
              <button
                @click="toggleIsAsking(sponsor.id)"
              >
                Excluir
              </button>
            </div>
          ...
          <ErrorComp
            v-if="editError && isEditing && sponsor.id === idOnFocus"
            :error="this.editError"
          />

          <div
            class="message-container"
            v-if="isAsking && sponsor.id === idOnFocus"
          >
            <h3>Tem certeza que deseja excluir esta parceira {{ sponsor.tradingName }}</h3>
...
import ViewHeader from '@/components/headers/ViewHeader.vue';
  ...
  export default {
    ...
    data() {
        return {
          title: 'Parceiras - B-Flow',
          headerTitle: 'Parceiras',
          headerLabel: 'Gerencie suas Parceiras',
          newRegisterLabel: 'Nova Parceira',
          listOfSponsors: [],
          listOfKeys: [],
          sponsorsLabels: [
            'ID',
    ...
    methods: {
      ...
      async getSponsors() {
        const response = await requestGet("/sponsors");
        if (!response || !response.length || response.length === 0) return null;
        this.listOfSponsors = await response.map(sponsor => {
          return {
            ...sponsor,
            createdAt: new Date(sponsor.createdAt).toLocaleDateString("pt-BR"),
            updatedAt: new Date(sponsor.updatedAt).toLocaleDateString("pt-BR"),
          };
        });
      },

      async deleteSponsor(id) {
        await requestDelete(`/sponsors/${id}`);
